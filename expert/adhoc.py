...
def total_sales_trends(df,
    ...
    if Business_line:
        df = df[df['Business Line'] == Business_line]
    if country:
    ...
    # COLORING
    #buf_pal = []
    #for i in range(len(colored_years)):
    #    buf_pal.append(palette[i % len(palette)])
    ...
    for year in colored_years:
        fig.add_trace(
            go.Scatter(
                ...
                text = [(f'{x:,.0f}') for x in df_months[df_months['Current Month'].dt.year == year][kpi]],
...
def total_sales_distribution(df,
                       kpi,
                       column_to_group_by,
                       palette=palette,
                       max_bars=None,
    ...
    fig.add_trace(
        go.Bar(
            ...
            text = [(f' {x:,.0f}   <i>{y:,.2f}%<i>') for x,y in zip(total_df[kpi], total_df[calc])],
...
def pareto_analysis(df,
    ...
    fig.add_trace(
        go.Bar(
            x = total_df[column_to_group_by],
            y = total_df[kpi],
            text = [(f' {x:,.0f}') for x in total_df[kpi]],
            textfont_color = total_df[f'quantiles_{kpi}'],
            textposition='outside',
            marker_color = total_df[f'quantiles_{kpi}'],
            marker_line = {
    ...
    fig.add_trace(
        go.Scatter(
            ...
            name = '',
            mode = 'text',
            text = [(f'{x:,.1f}%') for x in total_df[calc]],
